...
def objectif_pondere(emetteur, recepteur, beta, poids):
    """Identique a objectif(), sauf que la recompense pondere chaque
    referent par poids[i] au lieu de 1/N uniforme. L'entropie reste
    une moyenne non ponderee (le terme de regularisation n'est pas ce
    qu'on veut casser)."""
...
def continuer_sous_prior(e, r, beta, pas, lr, adam_eps, poids):
    activer(e, r)
...
def etat(e, r, msg=10, refs=(3, 4)):
    with torch.no_grad():
        ...
        return R, H_pleine, masse_34, H_binaire
...
if __name__ == "__main__":
    ...
    Rc, Hc, mc, Hbc = etat(e_ctrl, r_ctrl)
    ...
    Ra, Ha, ma, Hba = etat(e_asym, r_asym)
