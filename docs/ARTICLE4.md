...
- **The saddle-identity question reopened itself into a bigger one, and the bigger one is answered — with a residual mismatch still unexplained.** A natural, warmed-up trajectory (no cold start, no manual placement) was found that passes within 0.0002 of the closed-form saddle's own coordinates after roughly 260 real steps, confirmed twice independently. Its curvature coefficient stayed as unmeasurable as ever — sign flips under a hundred-step window change, an eleven-thousand-fold spread across disjoint sub-windows — but a residence-time measurement (how long a trajectory lingers near the point as its starting condition approaches the basin boundary) gave a clean answer the curvature fit never could: growth is logarithmic in distance from the boundary (R²=0.998), not the inverse-square-root divergence a degenerate fold predicts (R²=0.966, exponent nowhere near −1/2). That is the signature of an ordinary hyperbolic saddle, matching a Jacobian computed earlier the same session at the identical coordinates on the real system (two real eigenvalues, opposite sign, confirmed twice by different finite-difference methods) — two independent lines of evidence agreeing for the first time, rather than another failed measurement to add to the pile. One earlier piece of supporting evidence for the *other* structure didn't survive re-derivation: a prefactor agreement cited as confirming the degenerate-fold reading turned out to be an algebraic tautology (both configurations shared one timing constant, so their ratio reproduced a square root by construction, not by measurement) — retracted on sight once found. The honest reframing: the fixed-δ saddle and the point reached dynamically under drifting background mass were probably never the same *kind* of critical point to begin with — one hyperbolic, one degenerate — which makes "same saddle or neighbor" the wrong question, not an unresolved one. What doesn't close: the naive rate prediction for the hyperbolic saddle misses the measured escape rate by 13–20×, with no settled explanation (wrong time unit, wrong normal-form coordinate, and an invalid 1-D reduction of the coupled system are all still live candidates).
...
The question turned out to be aimed at the wrong distinction. What's resolved is that the fixed-δ saddle itself is an ordinary hyperbolic point — two independent measurements agree, a Jacobian and a residence-time law — while the point reached dynamically under drifting background mass shows the signature of a genuinely different, degenerate kind of critical point. "Same saddle or neighbor" assumed both were the same species; they probably aren't, which is a more useful answer than a yes or no would have been. What doesn't close: the escape rate predicted from first principles misses the measured one by more than an order of magnitude, for reasons not yet pinned down.
